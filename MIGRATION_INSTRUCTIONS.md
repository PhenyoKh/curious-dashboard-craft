# Calendar Integration Database Migration

## Problem
The calendar integration requires database tables that don't currently exist in Supabase, causing errors:
```
Error fetching Microsoft calendar integrations: relation "public.calendar_integrations" does not exist
```

## Solution: Manual Migration Application

Since the backend migration system is configured for local PostgreSQL but we're using Supabase, please follow these steps to apply the migration manually:
...
-- Add external calendar mapping to schedule_events (only if columns don't exist)
DO $$ BEGIN
    ALTER TABLE schedule_events 
    ADD COLUMN IF NOT EXISTS external_calendar_id UUID REFERENCES calendar_integrations(id) ON DELETE SET NULL;
EXCEPTION
...
DO $$ BEGIN
    ALTER TABLE schedule_events 
    ADD COLUMN IF NOT EXISTS sync_status VARCHAR(20) DEFAULT 'local' CHECK (sync_status IN ('local', 'synced', 'conflict', 'deleted', 'error'));
EXCEPTION
    WHEN others THEN null;
END $$;
...
DO $$ BEGIN
    ALTER TABLE schedule_events 
    ADD COLUMN IF NOT EXISTS external_last_modified TIMESTAMP WITH TIME ZONE;
EXCEPTION
    WHEN others THEN null;
END $$;

-- Create event sync mappings table for complex relationships
CREATE TABLE IF NOT EXISTS public.event_sync_mappings (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
...
CREATE INDEX IF NOT EXISTS idx_calendar_integrations_sync_status ON calendar_integrations(sync_status);
CREATE INDEX IF NOT EXISTS idx_calendar_integrations_next_sync ON calendar_integrations(user_id, sync_enabled, last_sync_at) 
WHERE sync_enabled = TRUE;

CREATE INDEX IF NOT EXISTS idx_schedule_events_external_id ON schedule_events(external_calendar_id, external_event_id) 
WHERE external_calendar_id IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_schedule_events_sync_status ON schedule_events(sync_status);

CREATE INDEX IF NOT EXISTS idx_event_sync_mappings_local_event ON event_sync_mappings(local_event_id);
CREATE INDEX IF NOT EXISTS idx_event_sync_mappings_external_event ON event_sync_mappings(calendar_integration_id, external_event_id);
CREATE INDEX IF NOT EXISTS idx_event_sync_mappings_conflicts ON event_sync_mappings(conflict_status) 
WHERE conflict_status != 'none';

CREATE INDEX IF NOT EXISTS idx_sync_conflicts_user_pending ON sync_conflicts(user_id, resolution_status) 
WHERE resolution_status = 'pending';

CREATE INDEX IF NOT EXISTS idx_sync_history_user_recent ON sync_history(user_id, started_at DESC);
CREATE INDEX IF NOT EXISTS idx_sync_history_integration_status ON sync_history(calendar_integration_id, sync_status);
...
After executing the SQL, you should see the following tables created:
...
Once the migration is applied, the calendar integration components should load without errors and you should be able to click "Connect Google Calendar" and "Connect Microsoft Calendar" buttons to test the OAuth flow.

## Alternative: Database Management Tools

You can also use database management tools like:
- **pgAdmin** or **DBeaver** connected to your Supabase database
- **Supabase CLI** (if available): `supabase db push`

## Verification

After applying the migration, the console errors should disappear and you should see:
- No more "relation 'calendar_integrations' does not exist" errors
- Calendar integration components load properly showing "Not Connected" state
- OAuth buttons are functional for testing