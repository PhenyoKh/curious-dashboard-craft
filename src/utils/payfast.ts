// PayFast signature utility - Ensures consistent signature generation
...
/**
 * Generate PayFast signature using exact same method as Edge Function
 * @param paymentData - Payment data object (including merchant_key)
...
export function generatePayFastSignature(paymentData: Record<string, string | number>, passphrase?: string): string {
  ...
  console.log('🔥 SIGNATURE DEBUG - Signature parameters (including merchant_key, excluding signature):', {
    ...
    signatureFieldExcluded: !Object.keys(signatureParams).includes('signature')
  });
  ...
  const orderedKeys = PAYFAST_FIELD_ORDER.filter(key => Object.prototype.hasOwnProperty.call(filteredParams, key));
  ...
  console.log('🔥 SIGNATURE DEBUG - Complete PHP urlencode-compatible signature construction:', {
    ...
    usesPlusForSpaces: signatureWithPassphrase.includes('+') && !signatureWithPassphrase.includes('%20'),
    stringBytes: Array.from(signatureWithPassphrase).map(c => c.charCodeAt(0))
  });
...
/**
 * Validate that our signature matches expected result
 * @param paymentData - Payment data
 * @param expectedSignature - Expected signature from server
 * @param passphrase - PayFast passphrase
